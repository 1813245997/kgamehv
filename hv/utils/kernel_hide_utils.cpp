...
namespace utils
{
	namespace kernel_hide
	{
		bool clear_mm_unloaded_drivers_instr(_In_ PWCHAR driver_name)
		{
			...
			if (!feature_init::mm_unloaded_drivers) {
				 
				return false;
			}

			if (!feature_init::mm_last_unloaded_driver) {
			 
				return false;
			}

			unloaders_count = *feature_init::mm_last_unloaded_driver;
			...
			{
				PMM_UNLOADED_DRIVER entry = &feature_init::mm_unloaded_drivers[i];
				if (entry->Name.Buffer && wcsstr(entry->Name.Buffer, target_name.Buffer))
				{
					...
					{
						feature_init::mm_unloaded_drivers[j] = feature_init::mm_unloaded_drivers[j + 1];
					}

					 
					RtlZeroMemory(&feature_init::mm_unloaded_drivers[unloaders_count - 1], sizeof(MM_UNLOADED_DRIVER));

					 
					(*feature_init::mm_last_unloaded_driver)--;
