...
#define ImFabs(X)          abs(X)
#define ImSqrt(X)           sqrtf(X)
#define ImFmod(X, Y)        fmodf_drv((X), (Y))
#define ImCos(X)            cosf_drv(X)
#define ImSin(X)            sinf(X)
#define ImAcos(X)           acosf(X)
#define ImAtan2(Y, X)       atan2f((Y), (X))
#define ImAtof(STR)       //  utils::math:: atof_kernel(STR)
#define ImCeil(X)           ceilf(X)
static inline float  ImPow(float x, float y)    { return powf_drv(x, y); }          // DragBehaviorT/SliderBehaviorT uses ImPow with either float/double and need the precision
static inline double ImPow(double x, double y)  { return 0; }
static inline float  ImLog(float x)             { return 0; }             // DragBehaviorT/SliderBehaviorT uses ImLog with either float/double and need the precision
static inline double ImLog(double x)            { return 0; }
static inline int    ImAbs(int x)               { return x < 0 ? -x : x; }
static inline float  ImAbs(float x)             { return abs(x); }
static inline double ImAbs(double x)            { return absd(x); }
